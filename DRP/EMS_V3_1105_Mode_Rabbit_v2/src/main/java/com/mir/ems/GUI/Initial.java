...
public class Initial extends JFrame {
	...
	public Initial() {
		...
		mode.setBounds(39, 190, 76, 15);
		panel_2.add(mode);
		
		//Thresdhold Mode
		mode2=new JComboBox<String>();
		mode2.addItem("none_threshold");
//		mode2.addItem("threshold");
		mode2.addItem("BEST");
		mode2.addItem("WORST");
		mode2.addItem("Negotiation");
		mode2.setSelectedIndex(0);
		mode2.setForeground(Color.BLACK);
		mode2.setBackground(Color.WHITE);
		mode2.setBounds(141, 190, 123, 21);
		panel_2.add(mode2);
...
//		if(comboBox.)
		if (userText.getText().equals("test") && new String(passText.getPassword()).equals("1234")) {
			...
			if (isLogin()) {
				...
				global.threshold_mode=mode2.getSelectedItem().toString();
