...
public class EnergyGraph3_no_threshold_noLine_Solution extends JPanel implements ExampleChart<XYChart> {
...
//	public static List<Double> totalEnergy = new ArrayList<Double>();
	...
	public EnergyGraph3_no_threshold_noLine_Solution() throws IOException{
...
//		thresholdSeries.isEnabled();
		...
		xyChart.addSeries("EMA's Energy", getEMAAXIS(), getEMAYAXIS()).setLineColor(new Color(40, 148, 40));
//		xyChart.addSeries(seriesName, xData, yData)
...
//		xyChart.getStyler().getSeriesColors();
//		xyChart.getStyler().setAxisTickMarksColor(Color.green);
//		xyChart.getStyler().setAxisTickLabelsColor(Color.green);
//		xyChart.getStyler().setChartFontColor(Color.green);
//		xyChart.getStyler().setPlotGridLinesColor(Color.green);
//		xyChart.getStyler().setSeriesLines(Color.GREEN);
...
//		yData.add(BASEWATT);
	...
	private List<Double> getEMAYAXIS() {
		...
		double sema1=global.emaProtocolCoAP.get("sEMA1").getPower();
		double sema2=global.emaProtocolCoAP.get("sEMA2").getPower();
		double sema3=global.emaProtocolCoAP.get("sEMA3").getPower();
...
//			totalEMAEnergy = totalEMAEnergy + (global.emaProtocolCoAP.get(key).getPower() / 100);			
		...
		try {
			...
			if(totalEMAEnergy>0) {
				...
				Double sema1th=global.SEMA_thresholdProfile.get("sEMA1");
				Double sema2th=global.SEMA_thresholdProfile.get("sEMA2");
				Double sema3th=global.SEMA_thresholdProfile.get("sEMA3");
				
				

				String data = str + "," + Double.toString(totalEMAEnergy) +","+Double.toString(sema1)+","+Double.toString(sema2)+","+Double.toString(sema3)+","+Double.toString(sema1th)+","+Double.toString(sema2th)+","+Double.toString(sema3th)+"\r\n";
