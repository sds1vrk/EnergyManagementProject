...
import com.mir.smartgrid.profile.emap.EnergyInformation;
import com.mir.ems.globalVar.global;
...
//	private static final int BASEWATT = 90;
//	private static final int BASEWATT = 135; //each ema 7
	private static final int BASEWATT = 21; //2개
	
//	private static final int YAXIS_TRANSFERENCE = 375;
	private static final int YAXIS_TRANSFERENCE = 2;
//	private static final int YAXIS_TRANSFERENCE = 10;
			...
			public void run() {
				...
				javax.swing.SwingUtilities.invokeLater(new Runnable() {
					...
					public void run() {
						...
						System.out.println("여기에서 totalEMAENergy?!"+totalEMAEnergy+"THRESHOLD?!"+global.THRESHOLD);
						
						if (totalEMAEnergy >= (global.THRESHOLD / 100) * global.EXPERIMENTPERCENT) {
							
							System.out.println("여기에서 totalEMAENergy?!"+totalEMAEnergy);
							if (overEnergy) {
								chart.getStyler().setPlotBackgroundColor(Color.RED);
								global.count_RED++;
								System.err.println("몇번?! 넘었냐?!"+global.count_RED);
							} else {
						...
						if (totalEMAEnergy >= (global.THRESHOLD / 100) * global.EXPERIMENTPERCENT) {
							...
							if (global.EXPERIMENTAUTODR) {
								System.out.println("autoDRCOUNT:");
								doEmergencyControl();
							}
	...
	private List<Double> getYAXIS() {
		...
		System.out.println("여기에서 THRESHOLD값:"+global.THRESHOLD);

		global.AVAILABLE_THRESHOLD = (global.THRESHOLD - (global.THRESHOLD / global.RESERVE_THRESHOLD_PERCENTAGE));
		
		System.out.println("여기에서 AVAILABLE_THRESHOLD값:"+global.AVAILABLE_THRESHOLD);
		
		global.RESERVE_THRESHOLD = global.THRESHOLD - global.AVAILABLE_THRESHOLD;
		
		System.out.println("여기에서 RESERVE_THRESHOLD값:"+global.RESERVE_THRESHOLD);

		yData.add(YAXIS_TRANSFERENCE * Math.sin(radians) + BASEWATT);
		//3375 (BASEWATT) + YAXIS_TRANSFERENCE * Math.sin(radians)
...
//			double threshold = global.THRESHOLD / global.EXPERIMENTNUM;
			...
			System.err.println("여기 threshold만큼 줄임"+threshold);
